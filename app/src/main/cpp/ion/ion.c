...
#define LOGTAG_LIBION  "LOGTAG_LIBION"

#define ION_LOGE(...)     __android_log_print(ANDROID_LOG_ERROR, LOGTAG_LIBION, __VA_ARGS__)
...
int ion_open() {
    ...
    if (fd < 0) ION_LOGE("open /dev/ion failed!\n");
...
static int ion_ioctl(int fd, int req, void* arg) {
    ...
    if (ret < 0) {
        ION_LOGE("ioctl %x failed with code %d: %s\n", req, ret, strerror(errno));
        return -errno;
    }
    return ret;
}
...
int ion_map(int fd, ion_user_handle_t handle, size_t length, int prot, int flags, off_t offset,
    ...
    if (ret < 0) return ret;
    if (data.fd < 0) {
        ION_LOGE("map ioctl returned negative fd\n");
        return -EINVAL;
    }
    tmp_ptr = mmap(NULL, length, prot, flags, data.fd, offset);
    if (tmp_ptr == MAP_FAILED) {
        ION_LOGE("mmap failed: %s\n", strerror(errno));
        return -errno;
    }
    *map_fd = data.fd;
    *ptr = tmp_ptr;
...
int ion_share(int fd, ion_user_handle_t handle, int* share_fd) {
    ...
    if (ret < 0) return ret;
    if (data.fd < 0) {
        ION_LOGE("share ioctl returned negative fd\n");
        return -EINVAL;
    }
    *share_fd = data.fd;
    return ret;
